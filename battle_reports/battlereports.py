...
#    r'.*?'
#    r'(Population killed: \d+\w? / \d+)?.*?'
#    r'(?P<fac>Factories destroyed: \d+)?.*?'
#    r'(?P<exp>Exploitations destroyed: \d+)?.*?'
#    r'(?P<civ>Civ\. level damage: [,\d]+)?.*?',
    re.M)
TECH_RGX = re.compile(
    r'([\d]+)%.*'
    ...
    re.M)
#BOMB_RGX = re.compile(
#    r'^Population killed: (?P<pop>\d+\w? / \d+)\. Factories \w+: (?P<fac>\d+)\. Exploitations \w+: (?P<exp>\d+)\. Civ\. level damage: (?P<civ>[,\d]+)',
#    re.M)
...
class BattleReport:
    ...
    def __parse(self, data):
        ...
        for unit, amount in atk_units.iteritems():
            self.atk_report = ''.join([self.atk_report, unit,
        ...
        for unit, amount in def_units.iteritems():
            self.def_report = ''.join([self.def_report, unit,
                                        amount['init'].rjust(27-len(unit)),
                                        amount['lost'].rjust(13), '\n'])
            if unit != 'Ground Armies' and unit != 'Carried Armies':
